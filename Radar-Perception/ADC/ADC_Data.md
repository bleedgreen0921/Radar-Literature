...
<img src="C:\Users\adminqiu\AppData\Roaming\Typora\typora-user-images\image-20250616103155614.png" alt="image-20250616103155614" style="zoom: 80%;" />

其中K是滤波器长度，fs是信号的采样频率，fl是较低的截止频率，b是带宽，K是滤波器参数索引。该滤波器的参数是较低的截止频率fl和隐式定义较高截止频率的带宽b。通过在慢时间和快时间方向上定义较低的截止频率和带宽，可以创建能够提取联合距离和速度特征的2D带通滤波器。

2D Sinc滤波器定义为

<img src="C:\Users\adminqiu\AppData\Roaming\Typora\typora-user-images\image-20250616103407688.png" alt="image-20250616103407688" style="zoom: 80%;" />

其中N和M是滤波器长度，f st s和f ft s是采样频率，f st l和f ft l是较低的截止频率，bst和bft分别是慢时间和快时间方向上的滤波器带宽。此外，w（n，m）是一个二维余弦加权函数。n代表慢时间，m代表快时间。

#### 2D Morlet Wavelets

当将信号转换到频域时，时间信息会丢失。这可以通过对时域信号加窗来克服。然而，较小的窗口尺寸意味着更高的时间分辨率，但代价是更差的频率分辨率，反之亦然。特别是对于时变信号，小波变换比傅里叶变换有几个优点，因为它们提供了时间和频率分辨率。由于雷达信号是高度时变的，因此提出了使用Morlet小波进行二维小波变换。、

2D Morlet小波定义为

<img src="C:\Users\adminqiu\AppData\Roaming\Typora\typora-user-images\image-20250616104458532.png" alt="image-20250616104458532" style="zoom:80%;" />

其中N和M是滤波器长度，σst和σft是标准偏差，f st c和f ft c是中心频率，f st s和f f t s分别是慢时间和快时间方向的采样频率。神经网络可以优化的滤波器参数是小波的中心频率和标准差。

![image-20250616105723108](C:\Users\adminqiu\AppData\Roaming\Typora\typora-user-images\image-20250616105723108.png)

#### 2D SincNet

2D SincNet在第一卷积层中使用2D sinc滤波器卷积。作为参数，该层分别采用滤波器长度、滤波器数量、采样频率、填充模式和慢速和快速时间方向的步长。虽然慢时间和快时间没有单独的滤波器，但需要明确提供慢时间Nst中的滤波器数量和快时间Nft中的滤波器的数量。据此，生成2D sinc滤波器的方式是，它们形成一个大小为Nst×Nft的相等网格，覆盖整个可观测距离多普勒域。该层中的可训练权重是较低的截止频率和慢速和快速时间方向的带宽。为了保证在两个滤波器维度上的训练相等，带宽和截止频率被归一化。

2D sinc滤波器层之后是池大小为8×2的MaxPool层。然后，使用50个大小为3×3的滤波器实现了一个常见的二维卷积层。卷积层以0.2的丢弃率跟在的dropout层之后。此外，在丢弃后，应用大小为4×2的最大池化来降低维度。然后，张量被压平并送入一个大小为32的密集层，然后是softmax分类器层。

#### 2D WaveConvNet

2D WaveConvNet（WCN）的设计类似于2D SincNet。只有第一卷积层由2D Morlet小波初始化，而不是使用2D sinc滤波器。此层所需的参数分别是滤波器长度、滤波器数量、采样频率、填充模式和慢速和快速时间方向的步幅。

与2D sinc滤波器类似，必须明确提供慢时间方向Nst上的滤波器数量以及快时间方向Nft上的滤波器的数量，以便在2D频域中将小波的频率响应作为网格均匀分布。

该层的可训练权重是慢时间维度和快时间维度的中心频率和标准偏差。同样在2D WCN中，可学习权重也被归一化。

#### Target Position Estimation Algorithm

![image-20250616135036908](C:\Users\adminqiu\AppData\Roaming\Typora\typora-user-images\image-20250616135036908.png)

1. #### **预处理阶段（Preprocessing）**

   - **Range FFT**

     对原始回波信号进行距离维FFT，生成距离剖面（Range Profile）。
   ...
   #### **2. 主处理循环（Key Processing）**

   ##### **步骤1-4：路径初始化与过滤**

   - **Step 1 (初始化累加器)**

     初始化累加器集合 *Ai*​={*a*1​,*a*2​,...,*aM*​}（*M*=256 为每帧的chirp数）。

   - **Step 2 (遍历距离峰值)**

     遍历所有chirp的距离维峰值，对峰值对应的距离门（Range Bin）累加计数。

   - **Step 3 (路径范围过滤)**

     设定阈值 *s*，筛选满足以下条件的峰值：*r*min​<range(fAi>s)<rmax​其中 *r*min​/*r*max​ 为计算的理论边界。

   - **Step 4 (路径排序)**

     将符合条件的峰值按路径长度降序排列，生成路径候选集 Ψ={*r*1′​,*r*2′​,...}。
   ...
   1. **Step 5 (选择主路径)**
      ...
      取候选集首元素 *r*1​=*r*1′​ 作为一次反射路径（Path #1）的长度。

   2. **Step 6 (水平波束扫描)**

      雷达以 2∘ 步进旋转，记录每个角度 *θ**s**i*​​ 的回波幅度值 *A**s**i*​​。

   3. **Step 7 (提取主方位角)**

      选取幅度峰值对应的方位角 *θ*1​=argmax(*A**s**i*​​)。
   ...
   6. **Step 11 (路径一致性验证)**
      ...
      - 判断条件：计算值是否均属于候选集
        - **Yes** → 输出目标坐标 (*xp*,*yp*)（**End**）。
        - **No** → 从 Ψ 中移除当前 *r*1′，更新集合 Ψ。
      - **循环终止**：若 Ψ 为空集（Ψ=∅），返回主流程重新开始搜索

**算法设计精髓**

1. **路径字典搜索**：通过几何约束（*r*min,*r*max）缩小搜索范围，提升效率。
2. **多路径交叉验证**：要求 *r*2 和 *rc* 必须同时存在于初始候选集 Ψ，避免误定位。
3. **宽波束适配**：旋转扫描替代MIMO硬件，降低系统成本（实测定位误差 **<0.15m**）。
